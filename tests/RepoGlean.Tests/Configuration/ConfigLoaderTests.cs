...
namespace RepoGlean.Tests.Configuration;

public sealed class ConfigLoaderTests : IDisposable
{
    private readonly string directory = Path.Combine(Path.GetTempPath(), $"repoglean-tests-{Guid.NewGuid():N}");
    ...
    public void Load_accepts_comments_trailing_commas_and_string_categories()
    {
        ...
        var config = Assert.IsType<RepoGleanConfig>(result.Config);
