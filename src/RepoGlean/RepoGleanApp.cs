...
public static class RepoGleanApp
{
    ...
    private static async Task WriteConfigAsync(RepoGleanConfig config, TextWriter output, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        var jsonContext = new RepoGleanJsonContext(serializerOptions);
        var json = JsonSerializer.Serialize(config, jsonContext.RepoGleanConfig);
    ...
    {
        output.WriteLine("RepoGlean - find regenerable Git-ignored development artifacts");
        output.WriteLine();
