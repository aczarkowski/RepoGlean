...
public sealed record ConfigLoadResult(RepoGleanConfig? Config, string? Error)
{
    public bool IsSuccess => Config is not null && Error is null;

    public static ConfigLoadResult Success(RepoGleanConfig config) => new(config, null);
...
{
    private static readonly RepoGleanJsonContext JsonContext = new(CreateSerializerOptions());

    public static string GetDefaultPath()
    {
        if (OperatingSystem.IsWindows())
        {
            return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "repoglean", "config.json");
        }
        ...
        return Path.Combine(configHome, "repoglean", "config.json");
    }
    ...
    internal static ConfigLoadResult LoadResolvedPath(string resolvedPath, bool isExplicit)
    {
        ...
        catch (Exception exception) when (exception is FileNotFoundException or DirectoryNotFoundException)
        {
            return isExplicit
                ...
                : ConfigLoadResult.Success(RepoGleanConfig.Default);
        }
    ...
    private static RepoGleanConfig Normalize(RepoGleanConfig config) => config with
    {
        Roots = config.Roots ?? [],
    ...
    private static bool Validate(RepoGleanConfig config, out string error)
    {
