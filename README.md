...
The published [JSON Schema](docs/configuration.schema.json) describes schema version 1. The loader accepts comments, trailing commas, and case-insensitive property names; generated output uses camelCase and named categories.
...
Custom rule IDs are stable lowercase dotted or hyphenated identifiers. Patterns and markers are repository-relative globs: `*` and `?` do not cross a path separator, while `**` spans path segments. A custom rule needs a standard category and at least one non-empty pattern, cannot reuse a built-in ID, and is never preselected in v1.
